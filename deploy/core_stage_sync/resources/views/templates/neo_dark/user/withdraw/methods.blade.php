                ...
                <div class="card card-bg">
                    <div class="card-body countdown-card">
                        ...
                        @if(!$savedWithdrawAddress)
                            <form action="{{ route('user.withdraw.save.address') }}" method="post" class="mb-4">
                                @csrf
                                <div class="form-group">
                                    <label class="form-label">@lang('Save USDT BEP20 Wallet Address')</label>
                                    <div class="input-group">
                                        ...
                                        <button type="submit" class="btn btn-primary">@lang('Save Address')</button>
                                    </div>
                                    <small class="text-muted">@lang('This address will be auto-detected when you create a withdrawal request')</small>
                                </div>
                            </form>
                        @else
                            <div class="alert alert-primary text-break">
                                <strong>@lang('Saved USDT BEP20 Address'):</strong> {{ $savedWithdrawAddress }}
                                <br>
                                <a href="{{ route('user.withdraw.address') }}" class="text-white text-decoration-underline">@lang('Change Address')</a>
                            </div>
                        @endif
                        <form action="{{route('user.withdraw.money')}}" method="post">
                            @csrf
                            <input type="hidden" name="withdraw_address" value="{{ $savedWithdrawAddress }}">
                            <div class="form-group">
                            ...
                            @if(!$savedWithdrawAddress)
                                <div class="alert alert-warning">
                                    @lang('Please save your USDT BEP20 wallet address first.')
                                </div>
                            @endif
                            ...
                            <button type="submit" class="btn btn-primary w-100 mt-3" {{ $savedWithdrawAddress ? '' : 'disabled' }}>@lang('Submit')</button>
                        </form>
                        @endif
                    </div>
                </div>
