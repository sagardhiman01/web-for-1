            ...
            <div class="row justify-content-center">
                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-bg mt-4">
                        <div class="card-body">
                            <h5 class="mb-3">@lang('Investment Access Code')</h5>
                            @if(!$isInvestmentCodeUnlocked)
                                <form action="{{ route('user.investment.code.redeem') }}" method="POST" class="row g-2">
                                    @csrf
                                    <div class="col-md-8">
                                        <input type="text" name="investment_code" class="form-control form--control" maxlength="8" placeholder="@lang('Enter 8-character code')" required>
                                    </div>
                                    <div class="col-md-4">
                                        <button type="submit" class="btn btn-primary w-100">@lang('Apply Code')</button>
                                    </div>
                                </form>
                                @if($activeCodeExpiresAt)
                                    <small class="text-muted d-block mt-2">
                                        @lang('Current code valid till'):
                                        <strong class="js-local-time" data-time-ms="{{ $activeCodeExpiresAtMs }}">
                                            {{ showDateTime($activeCodeExpiresAt) }}
                                        </strong>
                                    </small>
                                @endif
                            @else
                                @if($showInvestmentCodeActivatedMessage)
                                    <div class="alert border border--success mb-3" role="alert">
                                        <div class="alert__icon d-flex align-items-center text--success">
                                            <i class="fas fa-check-circle"></i>
                                        </div>
                                        <p class="alert__message mb-0">
                                            <span class="fw-bold">@lang('Code Activated')</span><br>
                                            <small>@lang('Applied code'): <strong>{{ $redeemedInvestmentCode }}</strong></small><br>
                                            @if($codeActivatedAt)
                                                <small>
                                                    @lang('Activated at'):
                                                    <strong class="js-local-time" data-time-ms="{{ $codeActivatedAtMs }}">
                                                        {{ showDateTime($codeActivatedAt) }}
                                                    </strong>
                                                </small>
                                            @endif
                                        </p>
                                ...
                                <div class="alert border border--primary mb-3" role="alert">
                                    <div class="alert__icon d-flex align-items-center text--primary">
                                        <i class="fas fa-clock"></i>
                                    </div>
                                    <p class="alert__message mb-0">
                                        <span class="fw-bold">@lang('Access Unlocked')</span><br>
                                        <small>
                                            @lang('Re-enter code after'):
                                            <strong class="js-local-time" data-time-ms="{{ $codeReEntryAtMs }}">
                                                {{ showDateTime($codeReEntryAt) }}
                                            </strong>
                                            |
                                            @lang('Re-entry required every 24 hours')
                                        </small>
                                        <br>
                                        <small>@lang('Time left'): <strong id="reEntryCountdown">--</strong></small>
                                    </p>
                                </div>
                                    ...
                                    <table class="table text-white mb-0">
                                        <thead>
                                            <tr>
                                                <th>@lang('Plan')</th>
                                                <th>@lang('Invested')</th>
                                                <th>@lang('Interest / Cycle')</th>
                                                <th>@lang('Cycle')</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @forelse($interestPlanSummary as $item)
                                                <tr>
                                                    <td>{{ __($item->plan_name) }}</td>
                                                    <td>{{ $general->cur_sym }}{{ showAmount($item->total_invest) }}</td>
                                                    <td>{{ $general->cur_sym }}{{ showAmount($item->interest_per_cycle) }}</td>
                                                    <td>{{ __($item->time_name) }}</td>
                                                </tr>
                                            @empty
                                                <tr>
                                                    <td colspan="100%" class="text-center">@lang('No active investment found')</td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                        <tfoot>
                                            <tr>
                                                <th colspan="2">@lang('Total Interest / Cycle')</th>
                                                <th>{{ $general->cur_sym }}{{ showAmount($interestPerCycleTotal) }}</th>
                                                <th>-</th>
                                            </tr>
                                        </tfoot>
                                    </table>
    ...
    @push('script')
        <script>
            ...
            (function($) {
                ...
                function createReEntryCountDown(elementId, targetMs) {
                    var target = parseTimeMs(targetMs);
                    if (!target) {
                        return;
                    }
                    ...
                    function tick() {
                        ...
                        if (diff <= 0) {
                            var doneElement = document.getElementById(elementId);
                            if (doneElement) {
                                doneElement.textContent = 'Ready to re-enter';
                            }
                            return;
                        }
                ...
                @if ($isInvestmentCodeUnlocked && $codeReEntryAtMs)
                    createReEntryCountDown('reEntryCountdown', {{ $codeReEntryAtMs }});
                @endif
            })(jQuery);
        </script>
    @endpush
