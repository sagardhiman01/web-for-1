...
function getContent($dataKeys, $singleQuery = false, $limit = null, $orderById = false)
{
    ...
    $resolver = function () use ($templateName, $dataKeys, $singleQuery, $limit, $orderById) {
        if ($singleQuery) {
            return Frontend::where('template_name', $templateName)
        ...
        if ($orderById) {
            return $article->where('data_keys', $dataKeys)->orderBy('id')->get();
        }

        return $article->where('data_keys', $dataKeys)->orderBy('id', 'desc')->get();
    };

    $cacheTtl = (int) config('performance.frontend_cache_seconds', 120);
    if ($cacheTtl <= 0) {
        return $resolver();
    }

    $cacheKey = sprintf(
        'frontend_content:%s:%s',
        $templateName,
        md5(json_encode([$dataKeys, $singleQuery, $limit, $orderById]))
    );
...
function getReferees($user, $maxLevel, $data = [], $depth = 1, $layer = 0)
{
    if ($user->allReferrals->count() > 0 && $maxLevel > 0) {
        foreach ($user->allReferrals as $under) {
            ...
            $userData['referral_code'] = $under->referral_code;
