...
class ManualGatewayController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $method->status               = 0;
        $method->gateway_parameters   = json_encode($this->prepareGatewayParameters($request));
    ...
    public function update(Request $request, $code)
    {
        ...
        $method->name                 = $request->name;
        $method->alias                = strtolower(trim(str_replace(' ', '_', $request->name)));
        $method->gateway_parameters   = json_encode($this->prepareGatewayParameters($request, $method));
        $method->supported_currencies = [];
        $method->crypto               = 0;
        $method->description          = $request->instruction;
        $method->form_id              = @$generate->id ?? 0;
    ...
    private function validation($request, $formProcessor)
    {
        $validation = [
            ...
            'qr_image'       => ['nullable', 'image', new FileTypeValidate(['jpg', 'jpeg', 'png', 'webp'])],
        ];
    ...
    public function uploadImage(Request $request)
    {
        ...
        $path = 'assets/images/manual_gateway';
        if (!file_exists($path)) {
            @mkdir($path, 0755, true);
        }

        try {
            $fileName = fileUploader($request->file('image'), $path);
            return response()->json([
                'data' => [
                    'link' => asset($path . '/' . $fileName),
                ],
            ]);
        } catch (\Exception $e) {
            return response()->json([
    ...
    private function prepareGatewayParameters(Request $request, Gateway $gateway = null): array
    {
        $parameters = [];
        ...
        $oldQr = $parameters['qr_image'] ?? null;
        ...
        if ($request->hasFile('qr_image')) {
            ...
            $parameters['qr_image'] = fileUploader($request->file('qr_image'), $path, null, $oldQr);
        } elseif ($oldQr) {
            $parameters['qr_image'] = $oldQr;
        }
